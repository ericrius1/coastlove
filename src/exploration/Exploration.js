...
export class Exploration {
 ...
 toggleJournal( open = ! this.journalOpen ) {
	...
	switchMode( mode ) {
		...
		app.game.toast( mode === 'plane' ? 'Marigold · easy flight · Space / C set height · release to hold' : mode === 'boat' ? 'At the helm · WASD to drive · V for cockpit view' : 'Safely ashore · follow your compass to meet an islander' );
 ...
 get canApproachCar(){
  ...
  const dx = targetPosition.x - p.position.x, dz = targetPosition.z - p.position.z;
		...
		this.find( '.exp-controls' ).textContent = p.mode === 'car' ? `W / S accelerate & reverse · A / D steer\n${Math.round(Math.abs(this.traffic.active?.speed||0)*3.6)} km/h · Space brake · E get out` : p.mode === 'plane' ? `A / D gentle turn · W faster · S slower\nSpace / C set height · release to hold · ${ Math.round( this.plane.speed * 3.6 ) } km/h` : p.mode === 'boat' ? 'WASD steer & throttle · Shift boost\n3 goes ashore · 2 takes flight' : 'WASD walk · Shift run · E enter car / listen\n1 summons boat · 2 takes flight';
