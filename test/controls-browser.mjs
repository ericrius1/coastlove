...
export function checkControls(app){
 ...
 try{
  ...
  const muted=app.audio.muted;press('KeyM');check(map.open&&e.paused&&!map.el.hidden,'M opens a modal map and pauses player controls');check(app.audio.muted===muted,'M leaves sound unchanged');end();
  ...
  for(const place of PLACES){
   e.toggleMap(true);map.select(place,true);map.travel();
  ...
  e.visit(PLACES.find(p=>p.id==='harbor'));press('Digit1');end();check(p.mode==='boat','numeric boat shortcut remains');
  ...
  for(const place of [PLACES.find(p=>p.id==='harbor'),PLACES.find(p=>p.major)]){
   e.visit(place);press('Digit2');end();check(p.mode==='plane','numeric plane shortcut remains');
  ...
  press('KeyM');end();press('KeyJ');check(!map.open&&e.journalOpen,'M map and J journal are mutually exclusive');end();
  ...
  e.visit(PLACES.find(place=>place.id==='harbor'));e.target=old.target;e.found=old.found;e.refresh();app.settings.timeOfDay=old.time;app.settings.timeSpeed=old.speed;
 }
}
