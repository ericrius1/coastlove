...
const keys = new Set();
let lookX = 0;
const input = { down: key => keys.has( key ), consumeLook: () => { const x = lookX; lookX = 0; return { x, y: 0 }; } };
...
assert.ok( trackpadPlane.heading > 0.06 && trackpadPlane.bank < 0, 'trackpad swipe right turns and banks right' );
lookX = - 30;
trackpadPlane.update( 1 / 60, input, camera );
...
lookX = 1000;
trackpadPlane.update( 1 / 60, input, camera );
assert.ok( trackpadPlane.heading <= 0.12 + 1e-6, 'a large pointer jump cannot snap the plane around' );
console.log( 'ok trackpad steering, bank, release and spike limit' );
