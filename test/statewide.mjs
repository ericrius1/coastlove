...
assert.ok(towns.buildingCount>300&&towns.buildingCount<1300);
for(const id of SHOWCASE_CITIES.filter(id=>id!=='los-angeles'))assert.ok(towns.groups.find(g=>g.town.id===id).boxes.length>65,`${id} is a substantial district`);
...
console.log('ok stable geography, 56 arrivals with city collisions,',towns.buildingCount,'buildings, bounded collider pool');
const tower=towns.groups.find(g=>g.town.id==='san-diego').boxes.find(b=>b.half.y>15);
...
for(const town of SETTLEMENTS.filter(t=>!['los-angeles','san-francisco'].includes(t.id))){
...
console.log('ok traffic follows the 28 stylized town visits, both lanes of every route remain drivable');
